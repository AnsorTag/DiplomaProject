...
If PostgreSQL already contains the project `tasks` database and
`public.tasks` table:

1. configure `.env` with its connection details;
2. run `scripts/migrate_tasks_multi_agent.py`.

The migration preserves existing task rows while adding the current
multi-agent fields, constraints, indexes, event table, and event trigger.
...
Create a project user, database, and the minimal base table expected by the
current migration:
...
described below. The migration completes the current schema.
...
This completes or updates the task schema and event trigger. It does not install
PostgreSQL or create the `tasks` database.
